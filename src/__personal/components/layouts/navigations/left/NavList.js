...
import { Box, Button, Divider, Drawer, ListItemButton, Typography, Zoom } from '@mui/material';
    ...
    const _clickBtn = async(name, link) => {
        if (name === 'tc') {
            ...
            if (__LOCATION.split('/', 2)[1] === 'tool-categories') {
                ...
                nav.push('psl/tool-categories/subscribed')
            }
            ...
            <Box height="100%" mx="16px" mt="12px" mb="24px"> 
                <Box height="100%" display="flex" flexDirection="column"> 
                    <Box height="100%" display="flex" flexDirection="column" gap="24px">
                        ...
                        <Box height="31px" width="100%" display="flex" alignItems="center" gap="8px">
                            <Box className="c-pointer" height="100%" width="100%" borderRadius="6px" bgcolor="#256EFF" color="#FFFFFF" display="flex" alignItems="center" justifyContent="center" 
                                sx={{':hover':{bgcolor:'#256EFF'} }}
                            >
                                <Box fontWeight={600} fontSize={leftBar.data ? 14 : 10} lineHeight="23px">Home</Box>
                            </Box>
                            <LightToolTip
                                title="Coming soon"
                                TransitionComponent={Zoom}
                            >
                                <Box sx={{display:leftBar.data ? 'block' : 'none'}} component={Button} height="100%" width="100%" borderRadius="6px" color="#52616B" display="flex" alignItems="center" justifyContent="center" >
                                    <Box fontWeight={400} fontSize={14} lineHeight="23px">Pro</Box>
                                </Box>
                            </LightToolTip>
                        </Box>
                        <Box>
                                    ...
                                    <Box display="flex" alignItems="center" justifyContent={ leftBar.data ? 'flex-start' : 'center'} width="100%" gap="12px">
                                        ...
                                        {leftBar.data && 
                                            <Typography  
                                                ...
                                                fontSize={14}
                                                lineHeight='23px'
                                                color={__LOCATION.split('/')[2] === t.ref || tabactive === t.name ? "primary.main" : "#313131"}
                                            >
                                                {t.name}
                                            </Typography>
                                        }
                    ...
                    <Box height="92px" display="flex" flexDirection="column" gap="4px">
                        ...
                        >
                            <Box display="flex" alignItems="center" justifyContent={ leftBar.data ? 'flex-start' : 'center'} width="100%" gap="8px">
                                <SupportIcons fill={tabactive === 'support' ? '#256EFF': '#89A594' }/>
                                {leftBar.data &&
                        ...
                        >
                            <Box display="flex" justifyContent="center" alignItems="center" gap="8px" width="100%">
                                <DownloadIcons fill="#FFFFFF" stroke="#3D77E9"/>
                                {leftBar.data && 
