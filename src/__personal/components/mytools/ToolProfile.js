...
export const ToolProfile = ({ tp, setTP }) => {
    const xsmHeight = useMediaQuery('(max-height:800px)');
    const { tool_state } = useContext(ToolContext)

    const redirectHandler = async() => {
        window.open(`${domain('mp').url}/sso/account/auth?ainfo=${encodeURIComponent(JSON.stringify(tool_state.__SESSION.data.ainfo))}&market_preview=${tp.data.tid}&platform=personal`, '_blank').focus()
    }
